...
class ExportPasswordList(object):
    ...
    def get_info(self) -> str:
        ...
        result = dict()
        result["统计数据"] = dict()
        result["统计数据"]["总计"] = len(self.passwords)
        result["统计数据"]["纯数字密码"] = len(self._numbers)
        result["统计数据"]["纯字母密码"] = len(self._letters)
        result["统计数据"]["大写字母密码"] = len(self._letters_upper)
        result["统计数据"]["小写字母密码"] = len(self._letter_lower)
        result["统计数据"]["数字字母组合密码"] = len(self._num_letters)
        result["统计数据"]["数字符号组合密码"] = len(self._num_signs)
        result["统计数据"]["字母符号组合密码"] = len(self._letter_signs)
        result["统计数据"]["字母数字符号混合密码"] = len(self._num_letter_signs)
        result["更新时间"] = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
        result["项目地址"] = "https://github.com/moonlightwatch/passwordlist"

        return json.dumps(obj=result, ensure_ascii=False, indent=4, sort_keys=True)
    ...
    def push(self) -> None:
        ...
        with open(file="index.md", mode="w", encoding="utf-8") as fp:
